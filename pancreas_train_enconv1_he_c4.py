...
sid        = 'he_c4'
dir_run = os.path.join(dir_out,'run.{}.{}'.format(sid,stamp1))
...
qqq_gray = np.mean(qqq_trn, axis=3, keepdims=True)
qqq_trn = np.concatenate((qqq_trn, qqq_gray), axis=3)
...
def get_enconv1(tf_input):
    with tf.variable_scope("enconv1", reuse=True):
        ...
        tf_4 = tf.nn.avg_pool(tf_3, ksize=[1,2,2,1], strides=[1,2,2,1], padding='SAME')
    return(tf_4)
...
def get_deconv_layers(qqq_enconv1):
    ...
    qqq_deconv_layers = np.zeros((1,ny*nf_enconv1,nx*nc,nf_input))
...
iii_enconv1 = np.argsort(np.mean(qqq_enconv1,axis=(0,1,2)))[::-1]
qqq_deconv1_layers = get_deconv_layers(qqq_enconv1[:,:,:,iii_enconv1])
#img_deconv1_layers = tensorflow_util.get_image_from_qqq(qqq_deconv1_layers)
tf_summary_image_deconv1_layers = tf.summary.image('deconv1_layers',qqq_deconv1_layers[:,:,:,0:nf_RGB])
...
for cc in range(nc):
    qqq_deconv1_cmp[0,0:ny,   nx*cc:(nx*cc+nx),] = qqq_trn[c0+cc,:,:,0:nf_RGB]
    qqq_deconv1_cmp[0,(ny):(ny+ny),nx*cc:(nx*cc+nx),] = qqq_deconv1[cc,:,:,0:nf_RGB]
